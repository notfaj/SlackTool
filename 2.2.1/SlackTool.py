...
def auto_update(tool_name='SlackTool'):
	...
	if server_checker[0] == 'True':
		if local_parser.getboolean('settings', 'auto_updating'):
			if not os.path.exists(f'{tool_name}.py'):
				print('Source code not found, downloading latest source code.')
				wget.download(f'https://raw.githubusercontent.com/Slackaduts/{tool_name}/main/{latest_version}/{tool_name}.py')
...
async def auto_adjusting_teleport(client):
	...
	while are_xyzs_within_threshold((await client.body.position()), original_position, 50):
...
async def main():
	...
	async def x_press():
		...
		background_clients = [c for c in clients if not c.is_foreground]
		await asyncio.gather(*[p.send_key(key=Keycode.X, seconds=0.1) for p in clients])

	async def space_press():
		logger.debug(f'{space_press_key} key pressed, sending spacebar press to all clients.')
		await asyncio.gather(*[p.send_key(key=Keycode.SPACEBAR, seconds=0.1) for p in clients])

	async def sync_locations():
		logger.debug(f'{sync_locations_key} key pressed, syncing client locations.')
		xyz = await p1.body.position()
		await asyncio.gather(*[p.teleport(xyz) for p in child_clients])
	...
	async def noclip_forward():
		nonlocal listener
		nonlocal forward_noclip_usage
		if forward_noclip_usage:
			return
		else:
			forward_noclip_usage = True
			await listener.remove_hotkey(Keycode.TWO)
			logger.debug(f'{forward_noclip_key} key pressed, noclipping forward.')
			selected_clients = [c for c in clients if c.is_foreground]
			if len(selected_clients) != 0:
				selected_client = selected_clients[0]
				selected_client_position = await selected_client.body.position()
				frontal_xyz = await calc_FrontalVector(selected_client, selected_client_position)
				await selected_client.teleport(xyz=frontal_xyz, move_after=False)
			await asyncio.sleep(0.9)
			await listener.add_hotkey(Keycode.TWO, noclip_forward)
			forward_noclip_usage = False
	...
	async def noclip_up():
		...
		if forward_noclip_usage:
			...
			forward_noclip_usage = True
			await listener.remove_hotkey(Keycode.ONE)
			logger.debug(f'{up_noclip_key} key pressed, noclipping up.')
			selected_clients = [c for c in clients if c.is_foreground]
			if len(selected_clients) != 0:
				selected_client = selected_clients[0]
				new_xyz = await calc_up_XYZ(selected_client, wizard_speed_constant=580)
				await selected_client.teleport(new_xyz, move_after=False)
			await asyncio.sleep(0.9)
			await listener.add_hotkey(Keycode.ONE, noclip_up)
			forward_noclip_usage = False
	...
	async def noclip_down():
		...
		if forward_noclip_usage:
			...
			forward_noclip_usage = True
			await listener.remove_hotkey(Keycode.THREE)
			...
			if len(selected_clients) != 0:
				...
				await selected_client.teleport(new_xyz, move_after=False)
			await asyncio.sleep(0.9)
			await listener.add_hotkey(Keycode.THREE, noclip_down)
			forward_noclip_usage = False
	...
	async def enable_hotkeys():
		...
		await listener.add_hotkey(Keycode[forward_noclip_key], noclip_forward)
		await listener.add_hotkey(Keycode[up_noclip_key], noclip_up)
		await listener.add_hotkey(Keycode[noclip_down_key], noclip_down)
	...
	async def disable_hotkeys():
		...
		await listener.remove_hotkey(Keycode[forward_noclip_key])
		await listener.remove_hotkey(Keycode[up_noclip_key])
		await listener.remove_hotkey(Keycode[noclip_down_key])
	...
	async def combat_loop():
		while True:
			await asyncio.sleep(0.1)
			if allow_combat == True:
				clients_in_combat = [p for p in clients if await CombatHandler(p).in_combat()]
				if sum([await CombatHandler(p).in_combat() for p in clients]) != 0:
					logger.debug('Client in combat, syncing clients and handling combat.')
					first_in_combat = clients_in_combat[0]
					xyz = await first_in_combat.body.position()
					not_in_combat_clients = [x for x in clients if not await CombatHandler(x).in_combat()]
					await asyncio.gather(*[p.teleport(xyz) for p in not_in_combat_clients])
					await asyncio.gather(*[p.send_key(key=Keycode.D, seconds=0.1) for p in not_in_combat_clients])
					await asyncio.sleep(1)
					clients_in_combat = [p for p in clients if await CombatHandler(p).in_combat()]
					await asyncio.gather(*[Spell_Logic(p).handle_combat() for p in clients_in_combat])
	...
	try:
		...
		while True:
			await asyncio.wait([combat_task, foreground_check_task, speed_multiplier_zone_check_task])
