...
class SlackQuester():
	...
	async def parse_name_talk_to(self,quest_name_path):
		...
		split2_qst = split1_qst[0].split(" in") #Parsing the quest name
	...
	async def find_quest_type(self,quest_name_path):
		...
		elif "Go To" in quest_msg:
			# goto quests
			quest_type = "talk to"
			return quest_type

		elif "Open " in quest_msg:
			quest_type = "collect"
			return quest_type
		else:
			quest_type = "talk to"
	...
	async def auto_collect(self):
		...
		print(entity)
		collect_key = await self.test_cords(popup_msgtext_path, entity, parsed_quest_info)
		while completed == False:
			for i in entity[collect_key]: # for every cord in the correct quest name item
				#print("tp under reagent " + str(key))
				await self.client.teleport(XYZ(i.x, i.y , i.z - 300), move_after = False)
				await asyncio.sleep(0.5)
				await self.client.teleport(XYZ(i.x, i.y , i.z- 300))
				await asyncio.sleep(0.5)
				...
				if can_Teleport == True:
					#await self.client.teleport(XYZ(i.x - 500, i.y - 500, i.z + 175), move_after= True)
					try:
						await teleport.navmap_tp(XYZ(i.x, i.y , i.z),minimum_distance_increment=500) #teleports to the npc
					except:
						await asyncio.sleep(0.1)
					...
					try:
						popup_text_path = await self.get_window_from_path(self.client.root_window,popup_msgtext_path)
						txtmsg = await popup_text_path.maybe_text()
						if await self.read_popup_title(parsed_quest_info) == True and "collect" in txtmsg.lower() or "open" in txtmsg.lower() :
	...
	async def test_cords(self, popup_msgtext_path, entity, parsed_quest_info):
		teleport = SlackTeleport(self.client)
		for key in entity.keys():
			for i in entity[key]:
				...
				await self.client.teleport(XYZ(i.x, i.y, i.z - 300))
				await asyncio.sleep(0.5)
				can_Teleport = await self.find_safe_entities_from(i, NULL , safe_distance=1700, is_mob=True) #checks if it's safe #change to nav map
				if can_Teleport == True:
					#await self.client.teleport(XYZ(i.x - 400, i.y - 400, i.z + 250)) #teleports up a little
					#await self.client.goto(i.x, i.y)
					#await self.client.teleport(XYZ(i.x - 500, i.y - 500, i.z + 175), move_after= True)
					try:
						await teleport.navmap_tp(XYZ(i.x, i.y , i.z),minimum_distance_increment=500) #teleports to the npc
					except:
