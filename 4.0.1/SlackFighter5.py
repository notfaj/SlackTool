...
from getcardnames import ReadCards
...
# Traps
SELECT_TRAPS = frozenset(['Hex', 'Disarming Trap', 'Curse', 'Spirit Trap', "Elemental Trap", 'Death Trap', 'Life Trap', 'Myth Trap', 'Storm Trap', 'Fire Trap', 'Backdraft', 'Feint', 'Ice Trap'])
...
buff_strategy = ['Ally Select Pierce','Ally Select Blade','AOE Blade',
				'Ally Select Pierce','Ally Select Blade','AOE Blade',
				'AOE Trap','Enemy Select Trap',
				'AOE Global',
				'AOE Hit Aura','AOE Defense Aura','AOE Minion','Ally Select Pierce','Ally Select Blade','AOE Blade',
				'Ally Select Pierce','Ally Select Blade','AOE Blade',
				'AOE Trap','Enemy Select Trap',
				'AOE Global',
				'AOE Hit Aura','AOE Defense Aura','AOE Minion',
				'AOE DOT', 'AOE Hit', 'Enemy Select DOT', 'Enemy Select Hit', 'Enemy Select Hit Divide']
# Basic types
...
class SlackFighter(CombatHandler):
	...
	async def handle_combat(self):
		DamageCalc = ReadCards(self.client)
		...
		while await self.is_fighting():
			...
			if self.cards:
				...
				await self.get_tc()
				self.bypass_strategy_to_kill, self.can_kill = await DamageCalc.handle_round(self.cards)
				print(self.can_kill)
	...
	async def handle_round(self):
		...
		if len([c for c in self.cards if self.card_names[c] == "Reshuffle"]) > len([c for c in self.cards if not self.card_names[c] == "Reshuffle" and not await c.is_treasure_card()]) and len([self.cards]) < 6 or [a for a in heal_types if a in priority_types] and not [c for c in self.cards if self.card_names[c] in AOE_HEALS and SELECT_HEALS and SELECT_SHIELDS] and len([self.cards]) < 5:
		...
		if self.can_kill:
			if selected_spell:
				logger.debug(f"Client {self.client.title} - Overriding {self.card_names[selected_spell]} to {self.card_names[self.bypass_strategy_to_kill]}")
				selected_spell = self.bypass_strategy_to_kill
			else:
				logger.debug(f"Client {self.client.title} - Overriding Casting{self.card_names[self.bypass_strategy_to_kill]}")
				selected_spell = self.bypass_strategy_to_kill
