...
class SlackQuester():
		...
		for key in entity.keys():
			while completed == False:
				for i in entity[key]:
					...
					await self.client.teleport(XYZ(i.x, i.y, i.z - 350))
					await asyncio.sleep(.5)
					can_Teleport = await self.find_safe_entities_from(i, NULL , safe_distance=2500, is_mob=True) # checks if safe to collect
					if can_Teleport == True:
						try:
							await teleport.navmap_tp(i)  # teleports to the npc
							#await asyncio.sleep(1)
							if await self.is_visible_by_path(path=npc_range_path):
	...
	async def tp_to_quest_mob(self):
		...
		if await sprinty.needs_health(health_percent=80):
			while attempts_to_find_closest_health_wisp < 25:
