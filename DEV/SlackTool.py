...
speed_multiplier = parser.getfloat('settings', 'speed_multiplier')
...
async def xyz_sync(foreground_client, background_clients, turn_after=True, debug=False):
	# syncs client XYZ up with the one in foreground, doesn't work across zones
	if debug:
...
async def main():
	...
	for p in clients:
		...
		await noclip_forward(p, speed_constant = 1)
	await enable_hotkeys()
