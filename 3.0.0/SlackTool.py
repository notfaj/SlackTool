...
import requests
import wget
from wizwalker import Keycode, HotkeyListener, ModifierKeys, WizWalker, utils
...
parser = ConfigParser()
parser.read(f'{tool_name}-config.ini')
...
def auto_update(tool_name=tool_name):
	# I should really do this, it uses weird fucking logic
	...
	if not os.path.exists('README.txt'):
		wget.download(f'https://raw.githubusercontent.com/Slackaduts/{tool_name}/main/README.txt')
		print('\n')
	elif len(read_webpage(f'https://raw.githubusercontent.com/Slackaduts/{tool_name}/main/README.txt')) != file_len('README.txt'):
...
async def mass_key_press(foreground_client, background_clients, pressed_key_name: str, key, duration=0.1, debug=False):
...
async def xyz_sync(foreground_client, background_clients, turn_after=True, debug=False):
	...
	if foreground_client:
		...
		xyz = await background_clients[0].body.position()
...
async def navmap_teleport(foreground_client, background_clients, mass_teleport: bool, debug=False):
...
async def toggle_speed(debug=False):
...
async def friend_teleport_sync(clients, debug: bool):
...
async def noclip_up(foreground_client, speed_constant = 580, speed_adjusted=True, down=False, debug=False):
...
async def noclip_forward(foreground_client, speed_constant = 580, speed_adjusted=True, backward=False, debug=False):
	...
	if foreground_client:
		...
		await foreground_client.teleport(xyz=frontal_xyz, move_after=False)
	else:
		logger.error('Foreground client does not exist, skipping teleport.')
...
async def main():
	...
	async def friend_teleport_sync_hotkey():
		await friend_teleport_sync(clients, debug=True)
	...
	async def assign_foreground_clients():
		...
		while True:
			foreground_client_list = [c for c in clients if c.is_foreground]
	...
	async def combat_loop():
		...
		async def async_combat(client):
			while True:
				...
				while (not await client.in_battle()) or not combat_status:
					await asyncio.sleep(1)
				if await client.in_battle() and combat_status:
	...
	await listener.set_global_message_loop_delay(0)
	await asyncio.sleep(0)
	# walker = ClientHandler(client_cls=NavigableClient)
	walker = WizWalker()
	clients = []
	...
	# teleport a bit forward to avoid the weird navmap tp speed issue when doing it for the first time
	# await asyncio.gather(*[noclip_forward(p, speed_constant = 1) for p in clients])
	await enable_hotkeys()
	logger.debug('Hotkeys ready!')
	logger.info('For friend teleports, be sure clients p2-p4 have client p1 friended and exclusively set with the purple gem icon.')
	try:
		...
		while True:
			await asyncio.wait([foreground_client_switching_task, assign_foreground_clients_task, speed_switching_task, combat_loop_task])

	finally:
		if speed_status:
			await asyncio.gather(*[p.client_object.write_speed_multiplier(client_speeds[p]) for p in clients])
		for p in clients:
			p.title = 'Wizard101'
			try:
